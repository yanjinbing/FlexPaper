...
public class Setup extends common{
	...
	public boolean pdftkEnabled(String path_to_pdftk){
		if ("/".equals(DIRECTORY_SEPARATOR) || ("\\".equals(DIRECTORY_SEPARATOR) && !"pdf2json".equals(path_to_pdftk))) {
			boolean result = false;
			if(isWin()){
				result = exec('"' + path_to_pdftk + '"' + " --version 2>&1");
			}else{
				result = exec(path_to_pdftk + " --version 2>&1");
			}
			if (result) {
